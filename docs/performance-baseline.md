...
| Successful (201) | 107             |
| Success Rate     | ~3.4%           |
| Failures         | 3,013 ETIMEDOUT |
| Mean Latency     | 5,920ms         |
| p99 Latency      | 9,801ms         |

Latency grew linearly from ~1.2s to ~8.4s even under light load (2 req/sec),
before heavy load caused complete collapse. Each request queued behind the
previous one — the event loop was fully occupied computing.
